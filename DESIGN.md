...
- **ChangesetValidationDrop** — drop a transparent validation/constraint from a changeset pipeline: `cs |> validate_required([:x])` → `cs` (pipe form becomes `Function.identity()`, exactly Mutare's CallRemoval delivery; non-pipe `validate_required(cs, …)` → `cs`). Targets the `Ecto.Changeset` validators and constraints (`validate_required`, `validate_length`, `validate_format`, `validate_number`, `validate_inclusion`, `validate_subset`, `unique_constraint`, `foreign_key_constraint`, …). A survivor means *no test exercises the rule this validation enforces*. Resolved through `Calls`, so it fires under `import Ecto.Changeset` (commonly `use`-bundled). Pipe-aware via `mutate/2`. **`:hook_drop`** is the sibling family for *deferred Repo-time hooks* (`prepare_changes`, `optimistic_lock`) — same drop mechanics, but they register code/locks the Repo runs at write time rather than a validation rule, so a survivor surfaces an untested side effect / concurrency guard, a different question (kept a distinct family).
- **RepoWrite** — two families on the persisting writes (`insert`/`update`/`delete`/`insert_or_update` + `!`). **`:persistence`** replaces the write with the equivalent *non-persisting* `Ecto.Changeset.apply_action/2`, surfacing **untested persistence**: `Repo.insert(cs)` → `apply_action(change(cs), :insert)`. The argument is normalised through `Ecto.Changeset.change/1`, which is *total* over both shapes a write accepts (a bare struct *and* a changeset), so the rewrite never breaks on the struct case; `apply_action` keeps the `{:ok, struct}`/`{:error, cs}` shape (and `apply_action!` raises like `insert!`), so the mutant diverges from the real write on *only* the success path (persistence + the DB-enforced constraints that fire only on the real call). It thus survives unless a test drives a successful write and asserts a persistence consequence. Excludes `insert_all`/`update_all` (bulk, no changeset). **`:on_conflict`** flips an explicit `on_conflict: :nothing` → `:raise` on an `insert`/`insert!` — the portable pair (both valid without a `conflict_target`, unlike `:replace_all`).
- **QueryTerminal** — `Ecto.Query.first`↔`last`: a same-arity rename of the query-edge terminals, behaviour-changing whenever the query can return more than one row. Family `:query_terminal`.
- *Rejected — RepoRaiseSwap.* The bang/non-bang twins (`insert`↔`insert!`, `get`↔`get!`) were considered and dropped: the swap is *shape*-changing (`{:ok, s}` ↔ bare `s`), so a downstream `{:ok, x} = …` match crashes — the mutant is almost always killed by a `MatchError`, not by a test asserting behaviour. The interesting "untested write success" gap it gestured at is captured precisely by `:persistence` above instead.
...
- **Boolean/expression fragments** (`where`, `having`, `on`, a dynamic `select`) — `wrap` is `&dynamic([bindings], &1)`. The plugin extracts `[bindings]` from the enclosing `from` (or the pipe stage's binding list) and re-declares them inside each `dynamic`. **One caveat for `on:`** (`Mutare.Ecto.Host.JoinOn`): a `^dynamic` is legal only as a join's *entire, top-level* on-expression. Ecto folds a join's multiple/implicit on-conditions into one `and` — so a join with two `on:` keys, or an `assoc(…)` join (implicit condition) plus an explicit `on:`, would put a `^dynamic` *operand* of that `and`, which Ecto rejects. Since the host wraps even the baseline branch, that corrupts mutant id 0 and aborts the run on a non-green baseline. So the host hosts an `on:` only when it is the join's sole, top-level condition; every other `on:` is left raw (its in-fragment mutants dropped, the whole-`from` `JoinType`/drop families still applying). `where`/`having` are each their own independent clause and never hit this.
...
**Only the positional bindings the author wrote are reordered.** The mutation tests a transposition the author could actually make — an explicit positional list they typed: a standalone/pipe `where([a, b], …)` / `having([a, b], …)` / join `on`, or a binding-list source `from([a, b] in q, …)`. The `[u, p]` the host *synthesizes* from `from u in User, join: p in Post` is **not** such a list — its order is fixed by clause position, not something the author could transpose — so it is left alone. Reordering it would invent a mistake the author cannot make and mutate a list we, not they, wrote (against *Mutate the Ecto an author writes*). The host therefore feeds the reorder catalog only the names from a written `[…]` (the standalone binding-list argument, or a `[…] in q` source), never names accumulated from scalar `in`-declarations or joins; named bindings (`comments: c`), addressed by name not position, are never moved either. The in-place families (`select`/`order_by`/`join`/…) already only ever reorder the explicit list in the call, so they need no such gate.
...
| **JoinType** | `:inner`↔`:left` (and `:left`↔`:right`, `*`→`:full`) | Changes result cardinality — a strong, killable mutation; only between join kinds the query is structurally valid under. **Equivalence-sensitive** (a *data* reason, not three-valued logic): the swap is equivalent whenever no orphan row exists — a mandatory/complete FK matches every row — so a survivor carries the "kill may require an orphan row" note (below), distinct from a test gap. |
| **Bound** | `limit`/`offset` `n`→`n±1`, drop `limit`/`offset` | Pinned-literal delivery. |
| **Literal arms** (`integer_literal`, `float_literal`, `string_literal`, `atom_literal`, `boolean_literal`) | a *non-pinned* literal inside a fragment, one family per type (`u.age > 18` → `19`; `u.score > 2.5` → `3.5`; `u.name == "ok"` → `""`/`"mutare"`; `u.status == :active` → `:mutare`; `… and true` → `… and false`) | The library owns in-fragment literals so it can keep them SQL-safe; each follows core's value convention (`Literal`/`FloatLiteral`/`StringLiteral`/`AtomLiteral`) but is decided here under SQL semantics. `boolean_literal` flips `true`↔`false` (worth mutating when the boolean is worth using — typically a non-comparison position); `nil` is left alone (NULL/absence). Named by type, not after `fragment(...)`. |
| **ClauseDrop** | remove a standalone/pipe clause *stage* — `q \|> where(…)` → `q`, `q \|> order_by(…)` → `q`, `q \|> join(…)` → `q`, … | The pipe/standalone twin of `from`-keyword clause drop. Pipe form becomes `Function.identity()` (Mutare's `CallRemoval` delivery), direct form collapses to the query argument. Family mirrors the keyword-form drop — `where`/`having` → `:filter_drop`, `limit`/`offset` → `:bound`, every other clause builder → `:clause_drop`. Resolved through `Calls` (`Mutare.Ecto.ClauseDrop`); pipe-aware via `mutate/2`. |
...
- **SQL-equivalence annotations.** A survivor may be *legitimately* unkillable for a **data** reason rather than a test gap, and the plugin tags such families with a per-family report note distinguishing it from a flat "your test is missing." Two reasons: (1) **three-valued logic** — a surviving `==`/`!=`, connective, `is_nil`, or NULLs-placement mutant on a nullable column needs a `NULL`/boundary fixture ("kill requires boundary/NULL data"); (2) **join cardinality** — a `JoinType` swap is equivalent whenever no orphan row exists (a mandatory/complete FK matches every row), so its survivor reads "kill may require an orphan row." Both are honest signal, not oversight.
...
3. **Full query catalog.** Ordering, Bound, Membership, JoinType, Aggregate (in `select`/`order_by` in place and in a hosted `having`), the in-fragment literal arms (`integer_literal`/`float_literal`/`string_literal`/`atom_literal`/`boolean_literal`), plus binding-reorder; the keyword-shorthand shape split and the `nil`-pair exclusion.
...
> **Implementation note (Milestone 4, shipped).** Every mutation is tagged with its SQL **family**, and each `{Mutare.Ecto, …}` entry reads `context.opts` (so all mutations now run through `mutate/2`, not `mutate/1`): `families:` filters the catalog (`Mutare.Ecto.Config`, with a loud error on an unknown name), `dialects:` gates the non-portable mutations (`like`↔`ilike` under `:postgres`; the `LEFT`↔`RIGHT` join under `:postgres`/`:mysql`). **Per-family naming** and **multi-repo** fall out of Mutare's existing `:as` convention — list the plugin twice with different `families:`/`as:` or `repo:`/`as:`. **Equivalence annotations** are a real per-mutant **report note**: a fourth core extension adds an optional `Mutare.Site` `note`, threaded from a host target's mutant (the new `%{node:, note:}` form) through `emit_hosted_site` and rendered in the survivor header (`… SURVIVED  — kill may require NULL/boundary data`) and the JSON report's `description`. The plugin attaches it to every equivalence-sensitive family (`:comparison`/`:connective`/`:null_predicate` — `Mutare.Ecto.Config.equivalence_note/1`), so a survivor there reads as honest signal, not a plain test gap; `Mutare.Ecto.equivalence_sensitive_families/0` plus the `:as` convention still lets a user *group* them under their own report name on top. "No silent equivalents" is inherent — the SQL-native catalog never emits an Elixir-equivalent mutation (`x * 1`) to inflate the denominator. *(Later: `:ordering_nulls` and `:join_type` joined the set once core accepted a `%Mutare.Mutator.Mutation{}` note on the plain `mutate/2` return, so the advisory now rides the whole-`from`/clause-macro rewrites too — `equivalence_note/1` returns per-family text, the three-valued note for the four logic families and a join-cardinality note ("kill may require an orphan row") for `:join_type`.)*
