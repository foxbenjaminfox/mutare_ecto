...
> **Implementation note (Milestone 4, shipped).** Every mutation is tagged with its SQL **family**, and each `{Mutare.Ecto, …}` entry reads `context.opts` (so all mutations now run through `mutate/2`, not `mutate/1`): `families:` filters the catalog (`Mutare.Ecto.Config`, with a loud error on an unknown name), `dialects:` gates the non-portable mutations (`like`↔`ilike` under `:postgres`; the `LEFT`↔`RIGHT` join under `:postgres`/`:mysql`). **Per-family naming** and **multi-repo** fall out of Mutare's existing `:as` convention — list the plugin twice with different `families:`/`as:` or `repo:`/`as:`. **Equivalence annotations** are realized through that same `:as` surfacing rather than a per-mutant report field: `Mutare.Ecto.equivalence_sensitive_families/0` names the families (`:comparison`/`:connective`/`:null_predicate`) whose survivors may need a `NULL`/boundary fixture, so running them under a dedicated `:as` (e.g. `:ecto_boundary_null`) makes the report distinguish "kill requires boundary/NULL data" from a plain test gap. "No silent equivalents" is inherent — the SQL-native catalog never emits an Elixir-equivalent mutation (`x * 1`) to inflate the denominator. A per-mutant report *note* (threaded from the host target through a new `Site` field) would be a further core extension; the `:as` surfacing covers the need without it.
