defmodule Mutare.Ecto.Ordering do
  ...
  is unwrapped first.
  """
  @spec flips(Macro.t()) :: [{atom(), Macro.t()}]
  ...
  def flips(value) when is_list(value) do
    ...
    |> Enum.flat_map(fn {pair, index} ->
      for {family, flipped} <- axis_flips(pair) do
        {family, List.replace_at(value, index, flipped)}
      end
    end)
  end
  ...
  # Every single-axis flip of one `direction: field` pair, tagged with its family.
  ...
  defp tag(_family, nil, _field), do: []
  defp tag(family, to, field), do: [{family, {AST.keyword_key(to), field}}]
end
