defmodule Mutare.Ecto.Host.Condition do
  ...
  # host owns. An expression is what the predicate catalog (`Mutare.Ecto.Fragment`) speaks, and
  # the host weaves it behind `dynamic/2`, which *is* the expression builder — it refuses a
  # filter's pairs ("Tuples can only be used in comparisons…") instead of translating them. A
  # root pin's interior is Elixir, sub-contracted to core (`Mutare.Ecto.Island`), and the host
  # weaves it pin-only, so Ecto still dispatches on the value the written pin carried (the
  # root-pin rule, `Mutare.Ecto.Host.Target`). A written list is neither.
  #
  ...
  # **is** a predicate in either shape: its own SQL catalog is empty, but its interior is
  # sub-contracted to core (`Mutare.Ecto.Island`) — and woven pin-only, with no `dynamic/2` and
  # so no re-declared bindings at all (the root-pin rule, `Mutare.Ecto.Host.Target`).
  ...
  first section):
    ...
    * `:predicate` — the host's to weave: an expression (the general expression builder's,
      woven behind `dynamic/2`), or a top-level `^cond` pin (dispatched on its runtime value,
      woven pin-only).
  ...
  """
  @type shape :: :predicate | {:keyword_filter, KeywordList.t()} | :pairless_list
  ...
  def shape(value) do
    ...
    if is_list(Mutare.AST.unwrap_literal(value)), do: list_shape(value), else: :predicate
  end
  ...
  defp binding_form(args) do
    with {binding_index, bindings} <- BindingList.find(args),
         ...
         :predicate <- shape(node) do
      %__MODULE__{node: node, index: index, bindings: bindings}
    else
      _ -> nil
    end
  end
  ...
  defp bindingless_form(args) do
    index = length(args) - 1
    node = Enum.at(args, index)
    if shape(node) == :predicate, do: %__MODULE__{node: node, index: index}, else: nil
  end
end
