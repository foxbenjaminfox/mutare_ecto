...
| `ast/query_call.ex` / `ast/from_call.ex` / `ast/binding_list.ex` / `ast/keyword_list.ex` | Normalized query-call, `from`, binding-list, and keyword/clause-list values; preserve written form while centralizing validation and reconstruction. `FromCall` is the **one** reader/rebuilder of the `from(source, clauses)` shape (`query.ex`, `subquery.ex`, `host.ex`, `host/target.ex`, and `host/routing.ex` via the identity-free `parse_args/1`); its `to_ast/1` owns the rule that an emptied clause list collapses to `from(source)` |
...
| `value_catalog.ex` | The **one** capability → catalog dispatch for a clause *value* delivered in place (`:ordering` → `Ordering.flips/1`, `:aggregate` → `Aggregate.swaps/1`, `:scalar` → `Scalar.swaps/2`) and the one statement of the ordering-position rule (`position/1`: a value is an `:ordering` position iff its key/macro carries the `:ordering` capability). `query.ex` (whole-`from`), `clause.ex` (standalone/pipe), and `subquery.ex` (a value-wrapped subquery's `select`) all read their `Surface` capabilities and rebuild through it, so the mapping and the rule can't drift between deliveries. `:combination` is not a value capability (it swaps a key/name), so each delivery applies it in its own shape |
