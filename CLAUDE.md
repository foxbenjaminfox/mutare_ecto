...
| `expression_walk.ex` | The generic single-point structural walker under the expression catalogs (`aggregate.ex`, `scalar.ex`). Stamps every mutant with **node-level attribution** (`Mutation.at/2`), so in-place deliveries (`query.ex`/`clause.ex`/`dynamic.ex`) report Sites at the mutated expression's own line — two identical `coalesce(a, b)`s in one `select` become individually `# mutare:ignore`-able (hosted relays discard the stamp structurally). Threads the ordering `position` to the catalogs and flips it itself inside an `over/2` window's `order_by:` option |
